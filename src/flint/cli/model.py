...
def pull(
    model_name: str = typer.Argument(
        ..., help="Name of the model to pull (e.g., llama3)"
    ),
    ...
    console.print(
        f" Pulling model [bold cyan]{model_name}[/bold cyan] via {backend.name}..."
    )
    ...
    if backend.name == "ollama":
        ...
        if result.returncode == 0:
            console.print(
                f" Successfully pulled [bold green]{model_name}[/bold green]."
            )
        else:
            console.print(
                f" Failed to pull [bold red]{model_name}[/bold red]. Is Ollama running?"
            )
            raise typer.Exit(1)
    else:
...
def run(
    ...
    prompt: str = typer.Argument(
        None, help="Prompt to send. If empty, starts interactive mode."
    ),
    backend_name: str = typer.Option(
        "ollama", "--backend", "-b", help="Backend to use (ollama, lmstudio, llamacpp)"
    ),
    ...
    async def _run():
        if prompt:
            console.print(
                f" [bold cyan]{model_name}[/bold cyan] (via {backend.name}): ", end=""
            )
            # Streaming output
            try:
            ...
            console.print(
                f" Starting interactive chat with [bold cyan]{model_name}[/bold cyan] via {backend.name}. Type 'exit' to quit."
            )
            while True:
