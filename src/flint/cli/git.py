...
@app.command("commit")
def generate_commit(
    auto_commit: bool = typer.Option(False, "--auto-commit", "-a", help="Automatically commit the generated message"),
    backend_name: str = typer.Option("ollama", "--backend", "-b", help="Backend to use"),
    model_name: str = typer.Option("qwen2.5:0.5b", "--model", "-m", help="Model to use")
):
    """
...
@app.command("review")
def code_review(
    staged_only: bool = typer.Option(False, "--staged-only", "-s", help="Only review staged changes"),
    backend_name: str = typer.Option("ollama", "--backend", "-b", help="Backend to use"),
    model_name: str = typer.Option("qwen2.5:0.5b", "--model", "-m", help="Model to use")
):
    """
